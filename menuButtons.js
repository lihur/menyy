...
const PopupMenu = imports.ui.popupMenu;								// apps menu
																	// item arc
const Clutter = imports.gi.Clutter;									// apps menu
																	// item arc
const DND = imports.ui.dnd;											// apps menu
																	// item arc
...
/*
 * ========================================================================= /*
 * name: AppGridButton @desc A button with an icon and label that holds app info
...
const CategoryGridButton = new Lang.Class({
    ...
    /*
	 * shellWorkspaceLaunch : function(params) { params = Params.parse(params, {
	 * workspace: -1, timestamp: 0 });
	 * 
	 * if (this._type == ApplicationType.APPLICATION) {
	 * this.app.open_new_window(params.workspace); } else if (this._type ==
	 * ApplicationType.PLACE) { if (this.app.uri) {
	 * this.app.app.launch_uris([this.app.uri], null); } else {
	 * this.app.launch(); } } else if (this._type == ApplicationType.RECENT) {
	 * Gio.app_info_launch_default_for_uri(this.app.uri,
	 * global.create_app_launch_context(0, -1)); }
	 * 
	 * this.actor.remove_style_pseudo_class('pressed');
	 * this.actor.remove_style_class_name('selected');
	 * 
	 * if (menyy.appsMenuButton) { if (menyy.appsMenuButton.menu.isOpen)
...
/*
 * ========================================================================= /*
 * name: AppListButton @desc A button with an icon and label that holds app info
 * for various @desc types of sources (application, places, recent) From GnoMenu
 * project
 * =========================================================================
 */
...
const AppButton = new Lang.Class({
    ...
    _init: function (app, button, appType, location) {
        ...
        this._draggable.connect('drag-begin', Lang.bind(this, function () {
        	...
        	// if (this._button) {
            // if (this._button._categoryWorkspaceMode ==
			// CategoryWorkspaceMode.CATEGORY)
            // this._button.toggleCategoryWorkspaceMode();
            // }
    ...
    _get_app_id: function(appType) {
    	...
    	if (appType == ApplicationType.APPLICATION) {
	    	...
	    	if (this.app.uri) {
	            ...
	            // global.log("menyy -> placeAppId -> path: " +
				// this.app.file.get_path());
	            // global.log("menyy -> placeAppId -> basename: " +
				// this.app.file.get_basename());
	            appId = this.app.file.get_path();
	        }
    	} else if (appType == ApplicationType.RECENT) {
    		appId = this.app.uri;
    	} else {
    ...
    activate: function(event) {
    	...
    	} else if (this._type == ApplicationType.TERMINAL) {
    		this.app.launch();
    	}
        this.parent(event);
...
// DELETEME!
// Removing the default behaviour which selects a hovered item if the space key
// is pressed.
// This avoids issues when searching for an app with a space character in its
// name.
/*
 * const BaseMenuItem = new Lang.Class({ Name: 'BaseMenuItem', Extends:
 * PopupMenu.PopupBaseMenuItem, _onKeyPressEvent: function (actor, event) { let
 * symbol = event.get_key_symbol();
 * 
 * if (symbol == Clutter.KEY_Return) { this.activate(event); return
 * Clutter.EVENT_STOP; } return Clutter.EVENT_PROPAGATE; } });
 */
/*
 * //User Home directories const DEFAULT_DIRECTORIES = [
 * GLib.UserDirectory.DIRECTORY_DOCUMENTS,
 * GLib.UserDirectory.DIRECTORY_DOWNLOAD, GLib.UserDirectory.DIRECTORY_MUSIC,
 * GLib.UserDirectory.DIRECTORY_PICTURES, GLib.UserDirectory.DIRECTORY_VIDEOS, ];
 */


/*
 * //Menu Category item class ARCMENU const CategoryMenuItem = new Lang.Class({
 * Name: 'CategoryMenuItem', Extends: BaseMenuItem,
 *  // Initialize menu item _init: function(button, category) { this.parent();
 * this._category = category; this._button = button; let name; if
 * (this._category) name = this._category.get_name(); //else // name =
 * _("Favorites");
 * 
 * this._icon = new St.Icon({ gicon: this._category.get_icon(), style_class:
 * 'menyy-popup-menu-icon', icon_size: APPLICATION_ICON_SIZE});
 * this.actor.add_child(this._icon); let categoryLabel = new
 * St.Label({style_class: 'menyy-popup-menu-label', text: name, y_expand: true,
 * y_align: Clutter.ActorAlign.CENTER }); this.actor.add_child(categoryLabel, {
 * expand: true }); this.actor.label_actor = categoryLabel; //let themeNode =
 * this.actor.get_theme_node(); //this.actor.set_style('padding-left: 0px;
 * border-left: 0px; -natural-hpadding: 0px; -minimum-hpadding: 0px;
 * margin-left: 0px; background-color: #0000ff;');
 *  },
 *  // Activate menu item (Display applications in category) activate:
 * function(event) { this._button._selectCategory(this._category);
 * this.parent(event); },
 *  // Set button as active, scroll to the button setActive: function(active,
 * params) { //if (active && !this.actor.hover) //
 ...
 * 
 * 
 * //arcmenu //Menu Place Shortcut item class /* const PlaceMenuItem = new
 * Lang.Class({ Name: 'PlaceMenuItem', Extends: BaseMenuItem,
 *  // Initialize menu item _init: function(button, info) { this.parent();
 * this._button = button; this._info = info; this._icon = new St.Icon({ gicon:
 * info.icon, icon_size: 16 }); this.actor.add_child(this._icon); this._label =
 * new St.Label({ text: info.name, y_expand: true, y_align:
 * Clutter.ActorAlign.CENTER }); this.actor.add_child(this._label, { expand:
 * true }); this._changedId = this._info.connect('changed', Lang.bind(this,
 * this._propertiesChanged)); },
 *  // Destroy menu item destroy: function() { if (this._changedId) {
 * this._info.disconnect(this._changedId); this._changedId = 0; } this.parent(); },
 *  // Activate (launch) the shortcut activate: function(event) {
 * this._info.launch(event.get_time()); this._button.menu.toggle();
 * this.parent(event); },
 *  // Handle changes in place info (redisplay new info) _propertiesChanged:
 * function(info) { this._icon.gicon = info.icon; this._label.text = info.name; },
 * });
 * 
 *  // Menu Application Item from ArcMenu const ApplicationMenuItem = new
 * Lang.Class({ Name: 'ApplicationMenuItem', Extends:
 * PopupMenu.PopupBaseMenuItem,
 *  // Initialize menu item _init: function(button, app) { this.parent();
 * this.app = app; this._button = button; this._iconBin = new St.Bin();
 * this.actor.add_child(this._iconBin);
 * 
 * let appLabel = new St.Label({style_class: "menyy-appLabel", text:
 * app.get_name(), y_expand: true, y_align: Clutter.ActorAlign.CENTER });
 * this.actor.add_child(appLabel, { expand: true }); this.actor.label_actor =
 * appLabel;
 * 
 * let textureCache = St.TextureCache.get_default(); let iconThemeChangedId =
 * textureCache.connect('icon-theme-changed', Lang.bind(this,
 * this._updateIcon)); this.actor.connect('destroy', Lang.bind(this, function() {
 * textureCache.disconnect(iconThemeChangedId); }));
 * this.actor.connect('popup-menu', Lang.bind(this, this._onKeyboardPopupMenu));
 * this._updateIcon(); this._menu = null; this._menuManager = new
 * PopupMenu.PopupMenuManager(this); this._menuTimeoutId = 0;
 * 
 * this._draggable = DND.makeDraggable(this.actor); this.isDraggableApp = true;
 * this._draggable.connect('drag-begin', Lang.bind(this, function () {
 * this._removeMenuTimeout(); Main.overview.beginItemDrag(this); }));
 * this._draggable.connect('drag-cancelled', Lang.bind(this, function () {
 * Main.overview.cancelledItemDrag(this); }));
 * this._draggable.connect('drag-end', Lang.bind(this, function () {
 * Main.overview.endItemDrag(this); })); },
 * 
 * get_app_id: function() { return this.app.get_id(); },
 * 
 * getDragActor: function() { return
 * this.app.create_icon_texture(APPLICATION_ICON_SIZE); },
 *  // Returns the original actor that should align with the actor // we show as
 * the item is being dragged. getDragActorSource: function() { return
 * this.actor; },
 *  // Activate menu item (Launch application) activate: function(event) {
 * this.app.open_new_window(-1); this._button.menu.toggle();
 * this.parent(event); },
 *  // Set button as active, scroll to the button setActive: function(active,
 * params) { if (active && !this.actor.hover) this._button.scrollToButton(this);
 * 
 * this.parent(active, params); },
 *  // Update the app icon in the menu _updateIcon: function() {
 * this._iconBin.set_child(this.app.create_icon_texture(APPLICATION_ICON_SIZE)); },
 * 
 * _removeMenuTimeout: function() { if (this._menuTimeoutId > 0) {
 * Mainloop.source_remove(this._menuTimeoutId); this._menuTimeoutId = 0; } },
 * 
 * _setPopupTimeout: function() { this._removeMenuTimeout(); this._menuTimeoutId =
 * Mainloop.timeout_add(AppDisplay.MENU_POPUP_TIMEOUT, Lang.bind(this,
 * function() { this._menuTimeoutId = 0; this.popupMenu(); return
 * GLib.SOURCE_REMOVE; })); GLib.Source.set_name_by_id(this._menuTimeoutId,
 * '[gnome-shell] this.popupMenu'); },
 * 
 * _onLeaveEvent: function(actor, event) { this._removeMenuTimeout(); },
 * 
 * popupMenu: function() { this._removeMenuTimeout();
 * 
 * if (this._draggable) this._draggable.fakeRelease();
 * 
 * if (!this._menu) { this._menu = new SecondaryMenu.AppItemMenu(this);;
 * this._menu.connect('activate-window', Lang.bind(this, function (menu, window) {
 * this.activateWindow(window); })); this._menu.connect('open-state-changed',
 * Lang.bind(this, function (menu, isPoppedUp) { if (!isPoppedUp)
 * this._onMenuPoppedDown(); })); let id = Main.overview.connect('hiding',
 * Lang.bind(this, function () { this._menu.close(); }));
 * this.actor.connect('destroy', function() { Main.overview.disconnect(id); });
 * 
 * this._menuManager.addMenu(this._menu); }
 * 
 * this.emit('menu-state-changed', true);
 * 
 * this.actor.set_hover(true); this._menu.popup();
 * this._menuManager.ignoreRelease();
 * 
 * return false; },
 * 
 * _onMenuPoppedDown: function() { this.actor.sync_hover();
 * this.emit('menu-state-changed', false); },
 * 
 * _onKeyboardPopupMenu: function() { this.popupMenu();
 * this._menu.actor.navigate_focus(null, Gtk.DirectionType.TAB_FORWARD, false); },
 * 
 * _onButtonPressEvent: function(actor, event) {
 * this.actor.add_style_pseudo_class ('active'); let button =
 * event.get_button(); if (button == 1) { this._setPopupTimeout(); } else if
 * (button == 3) { this.popupMenu(); return Clutter.EVENT_STOP; } return
 * Clutter.EVENT_PROPAGATE; },
 * 
 * _onButtonReleaseEvent: function (actor, event) { this._removeMenuTimeout();
 * this.actor.remove_style_pseudo_class ('active'); let button =
 * event.get_button(); if (button != 3) { this.activate(event); } return
 * Clutter.EVENT_STOP; },
 * 
 * _onTouchEvent: function (actor, event) { if (event.type() ==
 * Clutter.EventType.TOUCH_BEGIN) this._setPopupTimeout();
 * 
 * return Clutter.EVENT_PROPAGATE; },
 * 
 * _onDestroy: function() { this.parent(); this._removeMenuTimeout(); } });
 */


/*
 * //Menu shortcut item class const ShortcutMenuItem = new Lang.Class({ Name:
 * 'ShortcutMenuItem', Extends: BaseMenuItem,
 *  // Initialize the menu item _init: function(button, name, icon, command) {
 * this.parent(); this._button = button; this._command = command; this._icon =
 * new St.Icon({ icon_name: icon, style_class: 'popup-menu-icon', icon_size:
 * 16}); this.actor.add_child(this._icon); let label = new St.Label({ text:
 * name, y_expand: true, y_align: Clutter.ActorAlign.CENTER });
 * this.actor.add_child(label, { expand: true }); },
 *  // Activate the menu item (Launch the shortcut) activate: function(event) {
 * Util.spawnCommandLine(this._command); this._button.menu.toggle();
 * this.parent(event); } });
 */


/*
 * //Menu item to go back to category view (for arcmenu compatibility) const
 * BackMenuItem = new Lang.Class({ Name: 'BackMenuItem', Extends: BaseMenuItem,
 *  // Initialize the button _init: function(button) { this.parent();
 * this._button = button;
 * 
 * this._icon = new St.Icon({ icon_name: 'go-previous-symbolic', style_class:
 * 'popup-menu-icon', icon_size: 32}); this.actor.add_child(this._icon); let
 * backLabel = new St.Label({ text: _("Back"), y_expand: true, y_align:
 * Clutter.ActorAlign.CENTER }); this.actor.add_child(backLabel, { expand: true
 * }); },
 *  // Activate the button (go back to category view) activate: function(event) {
 * this._button._selectCategory(null); if (this._button.searchActive)
 * this._button.resetSearch(); this.parent(event); }, });
 */
...
/*
 * //Menu item which displays the current user const UserMenuItem = new
 * Lang.Class({ Name: 'UserMenuItem', Extends: BaseMenuItem,
 *  // Initialize the menu item _init: function(button) { this.parent();
 * this._button = button; let username = GLib.get_user_name(); this._user =
 * AccountsService.UserManager.get_default().get_user(username); this._userIcon =
 * new St.Icon({ style_class: 'popup-menu-icon', icon_size: USER_ICON_SIZE});
 * this.actor.add_child(this._userIcon); this._userLabel = new St.Label({ text:
 * username, y_expand: true, //font size changing? y_align:
 * Clutter.ActorAlign.CENTER, }); this.actor.add_child(this._userLabel, {
 * expand: true }); this._userLoadedId = this._user.connect('notify::is_loaded',
 * Lang.bind(this, this._onUserChanged)); this._userChangedId =
 * this._user.connect('changed', Lang.bind(this, this._onUserChanged));
 * this.actor.connect('destroy', Lang.bind(this, this._onDestroy));
 * this._onUserChanged(); },
 *  // Activate the menu item (Open user account settings) activate:
 * function(event) { Util.spawnCommandLine("gnome-control-center
 * user-accounts"); this._button.menu.toggle(); this.parent(event); },
 *  // Handle changes to user information (redisplay new info) _onUserChanged:
 * function() { if (this._user.is_loaded) { this._userLabel.set_text
 * (this._user.get_real_name()); if (this._userIcon) { let iconFileName =
 * this._user.get_icon_file(); let iconFile =
 * Gio.file_new_for_path(iconFileName); setIconAsync(this._userIcon, iconFile,
 * 'avatar-default'); } } },
 *  // Destroy the menu item _onDestroy: function() { if (this._userLoadedId !=
 * 0) { this._user.disconnect(this._userLoadedId); this._userLoadedId = 0; }
 * 
 * if (this._userChangedId != 0) { this._user.disconnect(this._userChangedId);
 * this._userChangedId = 0; } } });
 */
