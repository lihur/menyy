...
const IconsPage = new Lang.Class({
    ...
    _init: function(settings) {
         ...
         * Enable thumbnails
         */
        let thumbnailsFrame = new AM.FrameBox();
        let thumbnailsRow = new AM.FrameBoxRow();
        let thumbnailsLabel = new Gtk.Label({
            label: _("Enable thumbnails: "),
            use_markup: true,
            xalign: 0,
            hexpand: true
        });
        let thumbnailsSwitch = new Gtk.Switch({ halign: Gtk.Align.END });
        thumbnailsSwitch.set_active(this.settings.get_boolean('show-thumbnails'));
        thumbnailsSwitch.connect('notify::active', Lang.bind(this, function(check) {
            this.settings.set_boolean('show-thumbnails', check.get_active());
        }));
        thumbnailsRow.add(thumbnailsLabel);
        thumbnailsRow.add(thumbnailsSwitch);
        thumbnailsFrame.add(thumbnailsRow);
        this.add(thumbnailsFrame);
